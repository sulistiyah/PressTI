...
export const LoginAdmin = createAsyncThunk("api/admin/login", async(data, thunkAPI) => {
    try {
        // console.log(data)
        const response = await axios.post('http://34.203.73.249:8080/api/admin/login', {
            email: data.email,
...
export const getMe = createAsyncThunk("api/admin/getMe", async(_, thunkAPI) => {
    try {
        const response = await axios.get('http://34.203.73.249:8080/api/admin/my_admin');
        return response.data;
    } catch (error) {
        if(error.response){
            const message = error.response.data.message;
...
export const LogOut = createAsyncThunk("api/admin/LogOut", async() => {
    await axios.delete('http://34.203.73.249:8080/api/admin/logout', {
