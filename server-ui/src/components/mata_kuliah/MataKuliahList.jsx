...
function MataKuliahList() {
    ...
    const getMataKuliah = async () => {
        try {
            const response = await axios.get('http://localhost:8080/api/admin/mata_kuliah');
    ...
    const deleteMataKuliah = async (mataKuliahId) => {
        ...
        if(confirmDelete) {
            await axios.delete(`http://localhost:8080/api/admin/mata_kuliah/delete/${mataKuliahId}`)
            getMataKuliah()
        }
    }
