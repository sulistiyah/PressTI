...
function UserMahasiswaList() {
    ...
    const getUserMahasiswa = async () => {
        try {
            const response = await axios.get('http://localhost:8080/api/admin/user_mahasiswa');
    ...
    const deleteUserMahasiswa = async (userMahasiswaId) => {
        ...
        if(confirmDelete) {
            await axios.delete(`http://localhost:8080/api/admin/user_mahasiswa/delete/${userMahasiswaId}`)
            getUserMahasiswa()
        }
    }
