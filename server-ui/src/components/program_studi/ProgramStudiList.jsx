...
// axios.defaults.baseURL = 'http://34.203.73.249:8080/api/admin'
...
function ProgramStudiList() {
    ...
    const getProgramStudi = async () => {
        try {
            const response = await axios.get(`http://34.203.73.249:8080/api/admin/program_studi`);
    ...
    const deleteProgramStudi = async (programStudiId) => {
        ...
        if(confirmDelete) {
            await axios.delete(`http://34.203.73.249:8080/api/admin/program_studi/delete/${programStudiId}`)
            getProgramStudi()
        }
    }
