...
function UserDosenList() {
    ...
    const getUserDosen = async () => {
        try {
            const response = await axios.get('http://34.203.73.249:8080/api/admin/user_dosen');
    ...
    const deleteUserDosen = async (userDosenId) => {
        ...
        if(confirmDelete) {
            await axios.delete(`http://34.203.73.249:8080/api/admin/user_dosen/delete/${userDosenId}`)
            getUserDosen()
        }
    }
