...
function SetPresensiList() {
    ...
    const getSetPresensi = async () => {
        try {
            const response = await axios.get('http://34.203.73.249:8080/api/admin/set_presensi');
    ...
    const deleteSetPresensi = async (setPresensiId) => {
        ...
        if(confirmDelete) {
            await axios.delete(`http://34.203.73.249:8080/api/admin/set_presensi/delete/${setPresensiId}`)
            getSetPresensi()
        }
    }
