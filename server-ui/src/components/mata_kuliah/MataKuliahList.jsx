...
function MataKuliahList() {
    ...
    const getMataKuliah = async () => {
        try {
            const response = await axios.get('http://34.203.73.249:8080/api/admin/mata_kuliah');
    ...
    const deleteMataKuliah = async (mataKuliahId) => {
        ...
        if(confirmDelete) {
            await axios.delete(`http://34.203.73.249:8080/api/admin/mata_kuliah/delete/${mataKuliahId}`)
            getMataKuliah()
        }
    }
