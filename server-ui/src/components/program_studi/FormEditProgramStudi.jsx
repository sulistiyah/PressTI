...
function FormEditProgramStudi() {
    ...
    useEffect(() => {
        const getProgramStudiId = async() => {
            try {
                const response = await axios.get(`http://34.203.73.249:8080/api/admin/program_studi/${id}`)
    ...
    const updateProgramStudi = async(e) => {
        e.preventDefault()
        try {
            await axios.put(`http://34.203.73.249:8080/api/admin/program_studi/update/${id}`, {
                kodeProdi : kodeProdi,
