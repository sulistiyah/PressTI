...
function FormEditSetPresensi() {
    ...
    useEffect(() => {
        ...
        const getSetPresensiId = async() => {
            try {
                const response = await axios.get(`http://localhost:8080/api/admin/set_presensi/${id}`)
                ...
                const getMataKuliah = await axios.get('http://localhost:8080/api/admin/mata_kuliah')
                setMataKuliahList(getMataKuliah.data.data)
                setMataKuliah(response.data.data.mataKuliah)

                const getKelas = await axios.get('http://localhost:8080/api/admin/kelas')
                setKelasList(getKelas.data.data)
                setKelas(response.data.data.kelas)
                
                const getProgramStudi = await axios.get('http://localhost:8080/api/admin/program_studi')
    ...
    const updateSetPresensi = async(e) => {
        e.preventDefault()
        try {
            await axios.put(`http://localhost:8080/api/admin/set_presensi/update/${id}`, {
                tanggal : tanggal,
                jamMulai : jamMulai,
